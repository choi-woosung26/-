...
# h1 기본 크기(30px)에서 2pt 작게, h3도 같은 비율로 조정
st.markdown("""
<style>
...
st.title("📊 한국 주식 종목 검색기 — SMA 밴드 스캐너")
...
# 모든 키가 SMA이므로 딕셔너리 대신 함수로 단순화
def ma_name(key):
    return f"SMA{st.session_state.ma_params[key]}"

# ── 세션 상태 초기화 ─────────────────────────────────────────────
if 'ma_order' not in st.session_state or not set(st.session_state.ma_order).issubset(VALID_KEYS):
...
if 'close_dir' not in st.session_state or not set(st.session_state.close_dir.keys()).issubset(VALID_KEYS):
    st.session_state.close_dir = {
        ...
        'sma_short': 'above',   # SMA60 < 종가  (🟢 +종가)
        'sma_mid':   'above',   # SMA224 < 종가 (🟢 +종가)
        'sma_long':  'below',   # 종가 < SMA448 (🔴 -종가)
    }

if 'ma_params' not in st.session_state or not set(st.session_state.ma_params.keys()).issubset(VALID_KEYS):
...
for order_idx, key in enumerate(st.session_state.ma_order):
    ...
    val = st.sidebar.number_input(
        "SMA 기간", value=st.session_state.ma_params[key],
...
# 이 아래에서 order/params/close_dir 한 번만 선언해 이후 전체에서 재사용
...
# ── 메인: 드래그&드롭 순서 위젯 ─────────────────────────────────
st.subheader("📐 이동평균 배열 순서 — 드래그&드롭")
...
st.components.v1.html(drag_html, height=220, scrolling=False)

# ── 메인: 현재 조건 요약 ─────────────────────────────────────────
st.markdown(f"""
**검색 조건 (일봉 기준)**
- 📊 **배열**: {' · '.join(f"**SMA{params[order[i]]}** < **SMA{params[order[i+1]]}**" for i in range(len(order)-1))}
- 🎯 **종가**: {' · '.join(close_parts) if close_parts else '없음'}
...
(function() {{
  ...
  const chartInst=new Chart(ctx,{{
    data:{{
      labels:quarters,
      datasets:[{{type:'bar',data:opVals,backgroundColor:'transparent',borderWidth:0,yAxisID:'yLeft'}}]
    }},
    options:{{
      ...
      scales:{{
        ...
        yLeft:{{
          ...
          grid:{{color:c=>c.tick.value===0?'rgba(0,0,0,0.6)':'rgba(180,200,180,0.35)',lineWidth:c=>c.tick.value===0?2:1}},
    ...
    plugins:[{{
      ...
      afterDraw(chart){{
        ...
        function drawBar(cx,w,top,bot,color){{
          ...
          ctx.beginPath();
          if(top>bot){{
            ctx.moveTo(cx-w/2,yBot);ctx.lineTo(cx+w/2,yBot);
            ctx.lineTo(cx+w/2,yTop+r);ctx.quadraticCurveTo(cx+w/2,yTop,cx+w/2-r,yTop);
            ctx.lineTo(cx-w/2+r,yTop);ctx.quadraticCurveTo(cx-w/2,yTop,cx-w/2,yTop+r);
            ctx.lineTo(cx-w/2,yBot);
          }}else{{
            ctx.moveTo(cx-w/2,yTop);ctx.lineTo(cx+w/2,yTop);
            ctx.lineTo(cx+w/2,yBot-r);ctx.quadraticCurveTo(cx+w/2,yBot,cx+w/2-r,yBot);
            ctx.lineTo(cx-w/2+r,yBot);ctx.quadraticCurveTo(cx-w/2,yBot,cx-w/2,yBot-r);
        ...
        quarters.forEach((q,i)=>{{
          ...
          if(opVal!==null){{
            ...
            drawBar(opCX,opW,opTop,opBot,opVal>=0?'#3a9e5f':'#c0392b');
          }}
          if(dbtVal!==null){{
            drawBar(dbtCX,dbtW,zeroY,zeroY+dbtVal*debtPPU,'rgba(224,112,16,0.88)');
          }}
        }});
        ...
        quarters.forEach((q,i)=>{{
          ...
          if(opVal!==null){{
            ...
            if(barH>22){{
              ...
              ctx.fillText(opVal.toLocaleString(),opCX,opVal>=0?barTop+14:barBot-6);
            }}else{{
              ctx.fillStyle=opVal<0?'#8a1a10':'#1a5c30';
              ctx.fillText(opVal.toLocaleString(),opCX,opVal>=0?barTop-5:barBot+13);
            }}
          }}
          if(dbtVal!==null){{
            ...
            ctx.font="bold 10px 'Malgun Gothic',sans-serif";
            if(barH>22){{ctx.fillStyle='#ffffff';ctx.fillText(dbtVal.toFixed(1)+'%',dbtCX,dbtBot-5);}}
            else{{ctx.fillStyle='#8a3d00';ctx.fillText(dbtVal.toFixed(1)+'%',dbtCX,dbtBot+13);}}
          }}
        }});
  ...
  document.getElementById('{chart_id}').addEventListener('touchstart',function(e){{
    ...
    const elements=chartInst.getElementsAtEventForMode(
      {{clientX:touch.clientX,clientY:touch.clientY,target:this}},'index',{{intersect:false}},true
    );
...
# ── SMA 조건 검증 ────────────────────────────────────────────────
def check_sma_conditions(code_6, ma_order, ma_params, close_dir):
    max_period = max(ma_params[k] for k in ma_order)
...
def check_one(row_tuple, ma_order, ma_params, close_dir):
    idx, row = row_tuple
    result   = check_sma_conditions(row['종목코드'], ma_order, ma_params, close_dir)
...
if st.button("🔍 종목 검색 시작", use_container_width=True):
    if min_price >= max_price:
        ...
        # 검색 시점의 설정 스냅샷
        ma_order_snap  = list(order)
        ma_params_snap = dict(params)
        close_dir_snap = dict(close_dir)
        ...
        if data is None or data.empty:
            ...
            # 종목명 매핑 — map 후 미매핑분만 name 컬럼으로 보완 (단일 처리)
            ...
            if not results:
                ...
                fmt.update({c: '{:,.0f}' for c in ma_cols})
